use crate::utils::gmimc::{CYCLE_LENGTH, self};
use super::{
    AggPublicKey, BaseElement,DefaultRandomCoin,  ElementHasher, FieldElement, RingSigAir,
    PhantomData, ProofOptions, Prover, PublicInputs, TraceTable,
...
impl<H: ElementHasher> RingSigProver<H> {
    ...
    pub fn build_trace(
        ...
        let trace_length = SIGN_LENGTH + CYCLE_LENGTH * (key_path.len());
    
        // pad the trace length to a power of 2
        let padding_length = (trace_length+CYCLE_LENGTH).next_power_of_two();
        ...
        while step < CYCLE_LENGTH {
            gmimc::apply_round(&mut state[2..], step - 1);
            trace.update_row(step, &state);
            step += 1;
        } 
       ...
       let mut i = 0;
        while step < CYCLE_LENGTH * 2{
            gmimc::apply_round(&mut state[2..], i);
            trace.update_row(step, &state);
            step += 1;
            i += 1;
        ...
        trace.update_row(step, &state);
        step += 1;
        //println!("at {} the {}", step, state[2]);

        let mut i = 0;
        while step < CYCLE_LENGTH * 3 {
            gmimc::apply_round(&mut state[2..], i);
            trace.update_row(step, &state);
            step += 1;
            i += 1;
        ...
        while step < padding_length {
            let cycle_num = i / CYCLE_LENGTH;
            let cycle_pos = i % CYCLE_LENGTH;

            if cycle_pos <  CYCLE_LENGTH - 1 {
                gmimc::apply_round(&mut state[2..], cycle_pos);
                let rand:u64 = rand_value();
                state[0] = BaseElement::from(rand);
