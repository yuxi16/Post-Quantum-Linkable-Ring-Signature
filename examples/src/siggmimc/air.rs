...
use crate::utils::gmimc::{self, CYCLE_LENGTH};
...
impl Air for RingSigAir {
    ...
    fn get_assertions(&self) -> Vec<Assertion<Self::BaseField>> {
        ...
        let assertions = vec![
            ...
            // assert that the tag was generated correctly
    ...
    fn evaluate_transition<E: FieldElement + From<Self::BaseField>>(
        ...
        result.agg_constraint(0, hash_init_flag, (tag_flag - E::ONE) * (next[0] - E::ONE) * are_equal(current[2], next[2]));

        result.agg_constraint(0, hash_init_flag, (tag_flag - E::ONE) * (next[0] - E::ONE) * are_equal(current[3], next[3]));

        result.agg_constraint(1, hash_init_flag,  (tag_flag - E::ONE) *(next[0]) * are_equal(current[2], next[4]) );

        result.agg_constraint(1, hash_init_flag,  (tag_flag - E::ONE) *(next[0])  * are_equal(current[3], next[5]));
    ...
    fn get_periodic_column_values(&self) -> Vec<Vec<Self::BaseField>> {
        ...
        let mut v = vec![BaseElement::ONE;CYCLE_LENGTH-1];
        v.push(BaseElement::ZERO);
        result.push(v.to_vec());
        result.append(&mut gmimc::get_round_constants());
        result
    }
